 ...
 */
class AdminEcoGalerieController extends ModuleAdminController {

    public function __construct() {
        $this->bootstrap = true;
        $this->table = 'eco_galerie';
        $this->className = 'EcoGalerieModel';
    ...
    public function processSave() {
        if (Tools::isSubmit('submitArticle') && (Tools::getValue('id_article'))) {
            $article = new EcoGalerieModel(Tools::getValue('id_article'));
            $article->id_article = Tools::getValue('id_article');
            $infos_article = EcoGalerieModel::getArticleInfos($article->id_article);
            ...
            EcoGalerieModel::updateArticle($article->name_article, $article->desc_article, $article->date_article, $article->path_article, $article->path_mini, $article->id_article);
        }
        if (Tools::isSubmit('submitArticle') && !(Tools::getValue('id_article'))) {
            $article = new EcoGalerieModel();
            $article->name_article = Tools::getValue('name_article');
            $article->desc_article = Tools::getValue('desc_article');
            $article->date_article = Tools::getValue('date_article');
            ...
            $name_image = $infos_image_article['filename'];
            $article->path_article = Configuration::get('ECO_PATH_ARTICLE') . $name_image . '-' . $article->date_article . '.' . $extension;
            $extension_valide = array('jpg', 'JPG', 'jpeg', 'JPEG', 'png', 'PNG');
            if (($_FILES['path_article']['size'] > 999999999999) || (!in_array($extension, $extension_valide)))
